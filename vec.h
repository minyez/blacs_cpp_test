...
template <typename T>
inline void get_nr_nc_from_nested_vec(std::vector<vec<T>> nested_vec, int &nr, int &nc)
{
    nr = nested_vec.size();
    int nc_;
...
template <typename T>
inline void expand_nested_vector_to_pointer(const std::vector<vec<T>> &nested_vec, const int &nr, const int &nc, T* c, bool row_major = true)
{
    int nc_, nr_;
